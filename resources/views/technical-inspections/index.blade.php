<x-app-layout>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg">
                <div class="p-6">
                    <!-- Filtros -->
                    <div class="mb-6">
                        <form method="GET" action="{{ route('technical-inspections.index') }}" class="flex flex-wrap gap-4">
                            ...
                            <a href="{{ route('technical-inspections.index') }}" class="inline-flex items-center px-4 py-2 bg-gray-300 dark:bg-gray-600 border border-transparent rounded-md font-semibold text-xs text-gray-700 dark:text-gray-300 uppercase tracking-widest hover:bg-gray-400 dark:hover:bg-gray-500 focus:bg-gray-400 active:bg-gray-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
                                Limpar
                            </a>
                        </form>
                    </div>
                    @if($inspections->isEmpty())
                        <div class="text-center py-8">
                            ...
                            <div class="mt-6">
                                @can('edit service-orders')
                                <a href="{{ route('technical-inspections.create') }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 dark:bg-indigo-700 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-indigo-700 dark:hover:bg-indigo-600">
                                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
                                    </svg>
                                    {{ __('Nova Vistoria') }}
                                ...
                                @foreach($inspections as $inspection)
                                    <tr>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-gray-100">
                                            <a href="{{ route('technical-inspections.show', $inspection) }}" class="hover:text-indigo-600 dark:hover:text-indigo-400">
                                                {{ $inspection->number }}
                                            </a>
                                        </td>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $inspection->status_color }}">
                                                {{ $inspection->status_label }}
                                            </span>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                                            {{ $inspection->total_photos_count }} foto(s)
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <div class="flex items-center justify-end space-x-2">
                                                <!-- Visualizar -->
                                                <a href="{{ route('technical-inspections.show', $inspection) }}" 
                                                ...
                                                <button 
                                                   ...
                                                   @click="$dispatch('open-map-modal', { 
                                                       address: @js($inspection->address), 
                                                       coordinates: @js($inspection->coordinates) 
                                                   })"
                                                ...
                                                <a href="{{ route('technical-inspections.view-pdf', $inspection) }}" 
                                                   target="_blank"
                                                ...
                                                <!-- PDF - Download -->
                                                <a href="{{ route('technical-inspections.pdf', $inspection) }}" 
                                                ...
                                                <button 
                                                    ...
                                                    @click="$dispatch('open-delete-modal', { 
                                                        id: {{ $inspection->id }}, 
                                                        number: @js($inspection->number),
                                                        route: @js(route('technical-inspections.destroy', $inspection))
                                                    })"
    ...
    @push('scripts')
    @if(!empty($mapsApiKey))
    <script src="https://maps.googleapis.com/maps/api/js?key={{ $mapsApiKey }}&libraries=places&language=pt-BR&region=BR" async defer></script>
    @else
    <script>
