<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('contracts.show', $contract) }}" class="text-gray-600 hover:text-gray-900">
                ...
                <div class="p-6">
                    <form method="POST" action="{{ route('contracts.update', $contract) }}" enctype="multipart/form-data">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="title" value="{{ old('title', $contract->title) }}" required class="w-full border-gray-300 rounded-md">
                            @error('title')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="description" rows="3" class="w-full border-gray-300 rounded-md">{{ old('description', $contract->description) }}</textarea>
                        </div>

                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Data de Início</label>
                                <input type="date" name="start_date" value="{{ old('start_date', $contract->start_date?->format('Y-m-d')) }}" class="w-full border-gray-300 rounded-md">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Data de Término</label>
                                <input type="date" name="end_date" value="{{ old('end_date', $contract->end_date?->format('Y-m-d')) }}" class="w-full border-gray-300 rounded-md">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Valor</label>
                                <input type="number" name="value" step="0.01" value="{{ old('value', $contract->value) }}" class="w-full border-gray-300 rounded-md" placeholder="0.00">
                            </div>
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            @if($contract->file_path)
                                <p class="text-sm text-gray-600 mb-2">Arquivo atual: <a href="{{ route('contracts.download', $contract) }}" class="text-indigo-600 hover:text-indigo-900">Baixar</a></p>
                                <p class="text-sm text-gray-500 mb-2">Envie um novo arquivo para substituir o atual.</p>
                            @endif
                        ...
                        <div class="mb-4">
                            ...
                            <input type="datetime-local" name="signed_at" value="{{ old('signed_at', $contract->signed_at?->format('Y-m-d\TH:i')) }}" class="w-full border-gray-300 rounded-md">
                        </div>

                        <div class="mb-6">
                            <label class="block text-sm font-medium text-gray-700 mb-1">Observações</label>
                            <textarea name="notes" rows="3" class="w-full border-gray-300 rounded-md">{{ old('notes', $contract->notes) }}</textarea>
                        </div>

                        <div class="flex justify-end space-x-3">
                            <a href="{{ route('contracts.show', $contract) }}" class="px-4 py-2 bg-gray-300 text-gray-700 rounded-md hover:bg-gray-400">
                                Cancelar
                            </a>
