    ...
    <form wire:submit.prevent="save" class="space-y-6">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <x-input id="number" type="text" class="block mt-1 w-full bg-gray-50 dark:bg-gray-700" wire:model="number" readonly />
                        ...
                        @foreach($searchResults as $equipment)
                            ...
                            >
                                @if($equipment->photos && count($equipment->photos) > 0)
                                    <img src="/{{ ltrim($equipment->photos[0], '/') }}" alt="{{ $equipment->name }}" class="h-8 w-8 rounded object-cover mr-3">
                                @else
                                ...
                                @endif
                                <div>
                                    <div class="font-medium text-gray-900 dark:text-gray-100">{{ $equipment->name }}</div>
                                    <div class="text-sm text-gray-500 dark:text-gray-400">{{ $equipment->serial_number }}</div>
                                </div>
                            </button>
                        @endforeach
                    </div>
        ...
        @if(count($selectedEquipment) > 0)
            <div>
                <h3 class="text-lg font-medium text-gray-900 dark:text-gray-100 mb-3">{{ __('Equipamentos Selecionados') }}</h3>
                <div class="space-y-3">
                    @foreach($selectedEquipment as $index => $equipment)
                        <div class="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
                            <div class="flex items-center flex-1">
                                @if(isset($equipment['photo']) && $equipment['photo'])
                                    <img src="/{{ ltrim($equipment['photo'], '/') }}" alt="{{ $equipment['name'] }}" class="h-12 w-12 rounded object-cover mr-4">
                                @else
                                ...
                                @endif
                                <div class="flex-1">
                                    <h4 class="font-medium text-gray-900 dark:text-gray-100">{{ $equipment['name'] }}</h4>
                                    <p class="text-sm text-gray-500 dark:text-gray-400">Série: {{ $equipment['serial_number'] }}</p>
                                </div>
                            </div>
                            ...
                            <div class="flex items-center space-x-4">
                                ...
                                @endif
                                <button type="button" wire:click="removeEquipment({{ $index }})" class="text-red-600 dark:text-red-400 hover:text-red-900 dark:hover:text-red-300 p-2">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="flex items-center justify-end gap-4 mt-6 pt-4 border-t border-gray-200 dark:border-gray-700">
            ...
            <x-button-loading>
                {{ $equipmentRequest ? __('Atualizar') : __('Salvar') }}
            </x-button-loading>
        </div>
    </form>
</div>