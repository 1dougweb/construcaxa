                            ...
                            <div class="flex items-center justify-between">
                                <div>
                                    ...
                                    <p class="text-xs text-gray-600 dark:text-gray-400 mt-1">
                                        Estoque atual: <span class="font-medium">{{ number_format((float)($product->stock ?? 0), 2, ',', '.') }} {{ $product->unit_label }}</span>
                                    </p>
                                </div>
                                <button 
                                    type="button"
                            ...
                            <div class="absolute z-10 w-full bg-white dark:bg-gray-800 mt-1 rounded-md shadow-lg border border-gray-200 dark:border-gray-700 max-h-60 overflow-auto">
                                <ul class="py-1 text-base ring-1 ring-black ring-opacity-5 dark:ring-gray-700 focus:outline-none sm:text-sm">
                                    @foreach($availableProducts as $product)
                                        <li wire:click="selectProduct({{ $product->id }})" class="cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700 px-4 py-2 text-gray-900 dark:text-gray-100">
                                            <div class="flex items-center justify-between">
                                                <span>{{ $product->name }}</span>
                                                <span class="text-xs text-gray-500 dark:text-gray-400">Estoque: {{ number_format((float)($product->stock ?? 0), 2, ',', '.') }} {{ $product->unit_label }}</span>
                                            </div>
                                        </li>
                                    @endforeach
                                </ul>
