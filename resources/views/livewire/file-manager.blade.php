<div x-data="{ 
        ...
        handleDropOnMain(event) {
            ...
            if (event.dataTransfer.files && event.dataTransfer.files.length > 0) {
                @this.uploadMultiple('uploads', event.dataTransfer.files, () => {}, () => { 
        ...
        handleDropOnTarget(event, destinationPath) {
            ...
            if (sourcePath && this.internalDrag) {
                @this.call('moveItem', sourcePath, destinationPath);
                this.internalDrag = false;
            }
        },
        ...
        executeDelete() {
            if (this.itemToDelete) {
                @this.call('deleteItem', this.itemToDelete, this.isDirToDelete);
        ...
        <div class="flex flex-col sm:flex-row items-center space-y-2 sm:space-y-0 sm:space-x-4 w-full sm:w-auto">
            ...
            <div class="flex items-center p-1 bg-gray-100 dark:bg-gray-700/50 rounded-lg">
                <button wire:click="switchRoot('images')" 
                        class="px-3 py-1 text-xs font-bold rounded-md transition-all {{ $baseDirectory === 'images' ? 'bg-white dark:bg-gray-800 text-indigo-600 shadow-sm' : 'text-gray-500 hover:text-gray-700 dark:hover:text-gray-300' }}">
                    <i class="fi fi-rr-gallery mr-1"></i> Galeria
                </button>
                <button wire:click="switchRoot('storage')" 
                        class="px-3 py-1 text-xs font-bold rounded-md transition-all {{ $baseDirectory === 'storage' ? 'bg-white dark:bg-gray-800 text-indigo-600 shadow-sm' : 'text-gray-500 hover:text-gray-700 dark:hover:text-gray-300' }}">
            ...
            <nav class="flex items-center space-x-2 text-sm text-gray-600 dark:text-gray-400 font-medium overflow-x-auto whitespace-nowrap pb-2 sm:pb-0 hide-scrollbar w-full sm:w-auto">
                ...
                <button wire:click="upDirectory" 
                        ...
                        @drop.prevent.stop="$el.classList.remove('bg-gray-300', 'dark:bg-gray-600'); handleDropOnTarget($event, '{{ $this->breadcrumbs[count($this->breadcrumbs)-2]['path'] }}')"
                ...
                @foreach($this->breadcrumbs as $index => $crumb)
                    ...
                    <button wire:click="navigateTo('{{ $crumb['path'] }}')" 
                            ...
                            @drop.prevent.stop="$el.classList.remove('underline', 'text-indigo-600', 'dark:text-indigo-400'); handleDropOnTarget($event, '{{ $crumb['path'] }}')"
                    ...
                    @else
                    <span class="text-gray-900 dark:text-gray-100">{{ $crumb['name'] }}</span>
                    @endif
                @endforeach
            </nav>
        </div>
    ...
    <div class="p-4">
        ...
        @else
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
                ...
                <div draggable="true"
                     @dragstart="startDrag($event, '{{ $dir['path'] }}')"
                     ...
                     @drop.prevent.stop="$event.currentTarget.querySelector('.card-inner').classList.remove('ring-2','ring-indigo-400','!bg-indigo-100'); handleDropOnTarget($event, '{{ $dir['path'] }}')"
                     wire:click="enterDirectory('{{ $dir['name'] }}')"
                     class="group relative flex flex-col cursor-pointer select-none">

                    <!-- Botão Download (Pasta) -->
                    <button wire:click.stop="downloadDirectory('{{ $dir['path'] }}')"
                    ...
                    <!-- Botão Deletar -->
                    <button @click.stop="confirmDelete('{{ $dir['path'] }}', true)"
                    ...
                    <div class="mt-2 px-0.5">
                        <p class="text-xs font-semibold text-gray-800 dark:text-gray-200 truncate leading-tight" title="{{ $dir['name'] }}">{{ $dir['name'] }}</p>
                ...
                <div draggable="true"
                    ...
                    <button wire:click.stop="downloadFile('{{ $file['path'] }}', '{{ $file['name'] }}')"
                            class="absolute top-1 right-9 z-10 p-1.5 rounded-lg bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-400 hover:text-indigo-600 opacity-0 group-hover:opacity-100 transition-all shadow"
                            title="Baixar">
                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/></svg>
                    </button>
                    ...
                    <!-- Botão Deletar -->
                    <button @click.stop="confirmDelete('{{ $file['path'] }}', false)"
                            class="absolute top-1 right-1 z-10 p-1.5 rounded-lg bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-400 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-all shadow">
                        <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
                    </button>
                    ...
                    <div class="w-full relative pb-[100%]">
                        <div class="absolute inset-0">
                            @if($pickerMode && $file['is_image'])
                            <button type="button"
                                    wire:click.prevent="selectMedia('{{ $file['path'] }}', '{{ $file['url'] }}')"
                                ...
                                <img src="{{ $file['url'] }}" alt="{{ $file['name'] }}" loading="lazy" draggable="false"
                            ...
                            <a href="{{ $file['url'] }}" target="_blank" draggable="false" @dragstart.prevent
                            ...
                            <a href="{{ $file['url'] }}" target="_blank" draggable="false" @dragstart.prevent
                               class="w-full h-full rounded-2xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/80 flex flex-col items-center justify-center gap-2 group-hover:shadow-md transition-shadow">
                                ...
                                <span class="text-[11px] font-bold uppercase tracking-widest px-2 py-0.5 rounded-md bg-gray-200 dark:bg-gray-700 text-gray-500 dark:text-gray-400">{{ $file['extension'] }}</span>
                            </a>
                            @endif
                        </div>
                    </div>
                    ...
                    <!-- Rodapé -->
                    <div class="mt-2 px-0.5">
                        <p class="text-xs font-semibold text-gray-800 dark:text-gray-200 truncate leading-tight" title="{{ $file['name'] }}">{{ $file['name'] }}</p>
                        <p class="text-[10px] text-gray-400 dark:text-gray-500 leading-tight">{{ $file['size'] }}</p>
                    </div>
                </div>
                @endforeach
            </div>
    ...
    <div class="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm">
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-2xl p-6 w-full max-w-sm border border-gray-200 dark:border-gray-700">
            ...
            <div>
                ...
                @error('newDirectoryName') <span class="text-red-500 text-xs mt-1">{{ $message }}</span> @enderror
            </div>
