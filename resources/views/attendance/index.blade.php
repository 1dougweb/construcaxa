<x-app-layout>
    ...
    <script>
        const completedToday = {{ $todayEntries->count() >= 2 ? 'true' : 'false' }};
            ...
            if (typeof google === 'undefined' || typeof google.maps === 'undefined') {
                ...
                if (!googleMapsScriptAdded) {
                    ...
                    if (existingScript) {
                        ...
                        script.src = `https://maps.googleapis.com/maps/api/js?key={{ $googleMapsApiKey }}&libraries=geometry,places&loading=async`;
                        script.async = true;
                        script.defer = true;
                        script.onload = () => {
                            setTimeout(() => {
            ...
            confirmMarkerInstance.addListener('dragend', function(event) {
                ...
                if (modal && typeof Alpine !== 'undefined') {
                    ...
                    if (alpineData) {
                        ...
                        fetch(`{{ route('attendance.reverse-geocode') }}?lat=${newLat}&lng=${newLng}`)
                            .then(r => r.json())
                            .then(d => {
                                alpineData.address = d.address || null;
                                alpineData.loadingAddress = false;
