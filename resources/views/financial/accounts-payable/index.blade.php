<x-app-layout>
                            ...
                            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700" id="account-payables-table">
                                ...
                                <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                    @foreach($accountPayables as $accountPayable)
                                        <tr data-id="{{ $accountPayable->id }}" class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-gray-100">
                                                <a href="{{ route('financial.accounts-payable.show', $accountPayable) }}" class="hover:text-indigo-600 dark:hover:text-indigo-400">
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                <div class="flex items-center justify-end space-x-4">
                                                    <a href="{{ route('financial.accounts-payable.show', $accountPayable) }}" class="text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100" title="Visualizar">
                                                    ...
                                                    @can('manage finances')
                                                    <button 
                                                        onclick="openOffcanvas('account-payable-offcanvas'); loadAccountPayableForEdit({{ $accountPayable->id }});" 
    ...
    <x-offcanvas id="account-payable-offcanvas" title="Nova Conta a Pagar" width="w-full md:w-[700px]">
        <form method="POST" action="{{ route('financial.accounts-payable.store') }}" enctype="multipart/form-data" id="accountPayableForm">
            @csrf
    ...
    <script>
        function resetAccountPayableForm() {
            ...
            document.getElementById('accountPayableForm').action = '{{ route('financial.accounts-payable.store') }}';
        ...
        async function loadAccountPayableForEdit(id) {
            try {
                ...
                document.getElementById('accountPayableForm').action = `{{ url('financial/accounts-payable') }}/${accountPayable.id}`;
